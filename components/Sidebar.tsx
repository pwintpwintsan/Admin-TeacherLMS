...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, activeRole, checkPermission }) => {
  ...
  const superAdminItems = [
    { id: View.MY_CLASSES, label: 'My Hub', icon: Building2, color: '#3b82f6', category: 'accounts' },
    { id: View.COURSES_ADMIN, label: 'View Courses', icon: BookOpen, color: '#00a651', category: 'courses' },
    { id: View.ROLES_PERMISSIONS, label: 'Roster Permissions', icon: ShieldCheck, color: '#a855f7', category: 'accounts' },
    { id: View.EDIT_CERTIFICATES, label: 'Design Awards', icon: Award, color: '#f43f5e', category: 'certificates' },
    { id: View.ACCOUNT_CREATION, label: 'New Members', icon: UserPlus, color: '#00a651', category: 'accounts' },
    { id: View.RESOURCES, label: 'Hub Library', icon: FileSearch, color: '#6366f1', category: 'resources' },
  ].filter(item => checkPermission(item.category, 'view'));

  const teacherItems = [
    { id: View.MY_CLASSES, label: 'My classes', icon: LayoutDashboard, color: '#ec2027', category: 'accounts' },
    { id: View.COURSES_ADMIN, label: 'Manage Courses', icon: BookOpen, color: '#00a651', category: 'courses' },
  ...
  const menuItems = isStudent ? studentItems : (isAdmin ? adminItems : (isSuperAdmin ? superAdminItems : teacherItems));
  ...
  return (
    <div className="w-72 bg-[#292667] text-white flex flex-col hidden lg:flex h-full border-r-[12px] border-[#ec2027] shrink-0 overflow-hidden">
      <div className="p-6 flex-1 overflow-hidden flex flex-col justify-start">
        <div className="bg-white/5 p-4 rounded-[1.5rem] border-2 border-dashed border-white/20 relative group cursor-pointer overflow-hidden shrink-0 mb-6">
          <div className="flex items-center gap-4 relative z-10">
            <div className={`w-11 h-11 rounded-[1rem] flex items-center justify-center rotate-6 shadow-xl transition-transform group-hover:scale-110 ${isAdmin ? 'bg-[#ec2027]' : (isSuperAdmin ? 'bg-[#3b82f6]' : (isStudent ? 'bg-[#00a651]' : 'bg-[#fbee21]'))}`}>
              {isAdmin ? <Settings className="text-white" size={26} strokeWidth={3} /> : <Sparkles className={isStudent || isSuperAdmin ? "text-white" : "text-[#292667]"} size={26} strokeWidth={3} />}
            </div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#fbee21]">{isAdmin ? 'Main Center' : (isSuperAdmin ? 'Hub Admin' : (isStudent ? 'Learner Hub' : 'Educator'))}</p>
      ...
      <div className="p-6 shrink-0">
        <div className={`rounded-[1.5rem] p-4 flex items-center gap-4 border-4 shadow-2xl transition-transform hover:scale-105 cursor-pointer ${isAdmin ? 'bg-[#ec2027] border-[#fbee21]' : (isSuperAdmin ? 'bg-[#3b82f6] border-[#fbee21]' : (isStudent ? 'bg-[#00a651] border-[#fbee21]' : 'bg-[#292667] border-[#fbee21]'))}`}>
          <div className="relative shrink-0">
            <img 
              src={`https://picsum.photos/seed/${isAdmin ? 'admin-ub' : (isSuperAdmin ? 'super-ub' : (isStudent ? 'timmy' : 'u-profile'))}/64`} 
              className="w-12 h-12 rounded-xl border-4 border-white shadow-lg object-cover" 
              alt="Profile" 
            />
          </div>
          <div className="overflow-hidden">
            <p className="font-black text-white text-sm leading-tight truncate uppercase">{isAdmin ? 'Super Admin' : (isSuperAdmin ? 'Hub Manager' : (isStudent ? 'Learner Timmy' : 'Teacher Jane'))}</p>
            <p className={`${isAdmin || isSuperAdmin ? 'text-white/80' : 'text-[#fbee21]'} text-[10px] font-black uppercase mt-1 tracking-widest`}>{isAdmin ? 'Master Hub' : (isSuperAdmin ? 'Local Hub' : (isStudent ? 'Student Account' : 'Admin Access'))}</p>
          </div>
        </div>
      </div>
    </div>
